  ...
  return new Promise((resolve, reject) => {
    connection.connect((err) => {
      ...
      connection.query(`CREATE DATABASE IF NOT EXISTS ${process.env.DB_NAME || 'virtual_office'}`, (err) => {
        ...
        // Use the database
        connection.query(`USE ${process.env.DB_NAME || 'virtual_office'}`, (err) => {
          if (err) {
          ...
          connection.query(createTables, (err) => {
            ...
            const addColumnIfNotExists = (table, column, definition, callback) => {
              const checkQuery = `
              ...
              connection.query(checkQuery, [table, column], (err, results) => {
                if (err) {
                  return callback(err);
                }
            ...
            addColumnIfNotExists('users', 'phone_number', 'VARCHAR(20)', onMigrationComplete);
            addColumnIfNotExists('users', 'designation', 'VARCHAR(255)', onMigrationComplete);
            addColumnIfNotExists('users', 'bio', 'TEXT', onMigrationComplete);
            
            // Add columns to rooms table
            addColumnIfNotExists('rooms', 'background_image_url', 'VARCHAR(500)', onMigrationComplete);
            addColumnIfNotExists('rooms', 'background_layout', "ENUM('default', 'office', 'meeting', 'cozy', 'modern', 'custom') DEFAULT 'default'", onMigrationComplete);
          });
        });
      });
    });
